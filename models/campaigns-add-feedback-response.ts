/*
Mailchimp Marketing API

Mailchimp is a marketing platform for small businesses. It offers an all-in-one marketing solution that includes email marketing, ads, landing pages, and automation. With Mailchimp, businesses can design and send targeted campaigns, analyze their performance, and grow their audience.

The version of the OpenAPI document: 3.0.55
Contact: apihelp@mailchimp.com

NOTE: This file is auto generated by Konfig (https://konfigthis.com).
*/
import type * as buffer from "buffer"

import { ResourceLink } from './resource-link';

/**
 * A specific feedback message from a specific campaign.
 * @export
 * @interface CampaignsAddFeedbackResponse
 */
export interface CampaignsAddFeedbackResponse {
    /**
     * The individual id for the feedback item.
     * @type {number}
     * @memberof CampaignsAddFeedbackResponse
     */
    'feedback_id'?: number;
    /**
     * If a reply, the id of the parent feedback item.
     * @type {number}
     * @memberof CampaignsAddFeedbackResponse
     */
    'parent_id'?: number;
    /**
     * The block id for the editable block that the feedback addresses.
     * @type {number}
     * @memberof CampaignsAddFeedbackResponse
     */
    'block_id'?: number;
    /**
     * The content of the feedback.
     * @type {string}
     * @memberof CampaignsAddFeedbackResponse
     */
    'message'?: string;
    /**
     * The status of feedback.
     * @type {boolean}
     * @memberof CampaignsAddFeedbackResponse
     */
    'is_complete'?: boolean;
    /**
     * The login name of the user who created the feedback.
     * @type {string}
     * @memberof CampaignsAddFeedbackResponse
     */
    'created_by'?: string;
    /**
     * The date and time the feedback item was created in ISO 8601 format.
     * @type {string}
     * @memberof CampaignsAddFeedbackResponse
     */
    'created_at'?: string;
    /**
     * The date and time the feedback was last updated in ISO 8601 format.
     * @type {string}
     * @memberof CampaignsAddFeedbackResponse
     */
    'updated_at'?: string;
    /**
     * The source of the feedback.
     * @type {string}
     * @memberof CampaignsAddFeedbackResponse
     */
    'source'?: CampaignsAddFeedbackResponseSourceEnum;
    /**
     * The unique id for the campaign.
     * @type {string}
     * @memberof CampaignsAddFeedbackResponse
     */
    'campaign_id'?: string;
    /**
     * A list of link types and descriptions for the API schema documents.
     * @type {Array<ResourceLink>}
     * @memberof CampaignsAddFeedbackResponse
     */
    '_links'?: Array<ResourceLink>;
}

type CampaignsAddFeedbackResponseSourceEnum = 'api' | 'email' | 'sms' | 'web' | 'ios' | 'android'


