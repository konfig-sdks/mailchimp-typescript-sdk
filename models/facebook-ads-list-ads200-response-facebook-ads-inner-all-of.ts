/*
Mailchimp Marketing API

Mailchimp is a marketing platform for small businesses. It offers an all-in-one marketing solution that includes email marketing, ads, landing pages, and automation. With Mailchimp, businesses can design and send targeted campaigns, analyze their performance, and grow their audience.

The version of the OpenAPI document: 3.0.55
Contact: apihelp@mailchimp.com

NOTE: This file is auto generated by Konfig (https://konfigthis.com).
*/
import type * as buffer from "buffer"

import { RecipientsProperty1 } from './recipients-property1';
import { ReportSummaryProperty } from './report-summary-property';

/**
 * 
 * @export
 * @interface FacebookAdsListAds200ResponseFacebookAdsInnerAllOf
 */
export interface FacebookAdsListAds200ResponseFacebookAdsInnerAllOf {
    /**
     * Unique ID of an Outreach.
     * @type {string}
     * @memberof FacebookAdsListAds200ResponseFacebookAdsInnerAllOf
     */
    'id'?: string;
    /**
     * The ID used in the Mailchimp web application. For example, for a `regular` outreach, you can view this campaign in your Mailchimp account at `https://{dc}.admin.mailchimp.com/campaigns/show/?id={web_id}`.
     * @type {number}
     * @memberof FacebookAdsListAds200ResponseFacebookAdsInnerAllOf
     */
    'web_id'?: number;
    /**
     * Title or name of an Outreach.
     * @type {string}
     * @memberof FacebookAdsListAds200ResponseFacebookAdsInnerAllOf
     */
    'name'?: string;
    /**
     * The type of outreach this object is.
     * @type {string}
     * @memberof FacebookAdsListAds200ResponseFacebookAdsInnerAllOf
     */
    'type'?: FacebookAdsListAds200ResponseFacebookAdsInnerAllOfTypeEnum;
    /**
     * The status of this outreach.
     * @type {string}
     * @memberof FacebookAdsListAds200ResponseFacebookAdsInnerAllOf
     */
    'status'?: FacebookAdsListAds200ResponseFacebookAdsInnerAllOfStatusEnum;
    /**
     * Outreach report availability. Note: This property is hotly debated in what it _should_ convey. See [MCP-1371](https://jira.mailchimp.com/browse/MCP-1371) for more context.
     * @type {boolean}
     * @memberof FacebookAdsListAds200ResponseFacebookAdsInnerAllOf
     */
    'show_report'?: boolean;
    /**
     * The date and time the outreach was created in ISO 8601 format.
     * @type {string}
     * @memberof FacebookAdsListAds200ResponseFacebookAdsInnerAllOf
     */
    'create_time'?: string;
    /**
     * The date and time the outreach was started in ISO 8601 format.
     * @type {string}
     * @memberof FacebookAdsListAds200ResponseFacebookAdsInnerAllOf
     */
    'start_time'?: string;
    /**
     * The date and time the outreach was last updated in ISO 8601 format.
     * @type {string}
     * @memberof FacebookAdsListAds200ResponseFacebookAdsInnerAllOf
     */
    'updated_at'?: string;
    /**
     * The date and time the outreach was canceled in ISO 8601 format.
     * @type {string}
     * @memberof FacebookAdsListAds200ResponseFacebookAdsInnerAllOf
     */
    'canceled_at'?: string;
    /**
     * The date and time the outreach was (or will be) published in ISO 8601 format.
     * @type {string}
     * @memberof FacebookAdsListAds200ResponseFacebookAdsInnerAllOf
     */
    'published_time'?: string;
    /**
     * If this outreach targets a segment of your audience.
     * @type {boolean}
     * @memberof FacebookAdsListAds200ResponseFacebookAdsInnerAllOf
     */
    'has_segment'?: boolean;
    /**
     * 
     * @type {ReportSummaryProperty}
     * @memberof FacebookAdsListAds200ResponseFacebookAdsInnerAllOf
     */
    'report_summary'?: ReportSummaryProperty;
    /**
     * 
     * @type {RecipientsProperty1}
     * @memberof FacebookAdsListAds200ResponseFacebookAdsInnerAllOf
     */
    'recipients'?: RecipientsProperty1;
    /**
     * The URL of the thumbnail for this outreach.
     * @type {string}
     * @memberof FacebookAdsListAds200ResponseFacebookAdsInnerAllOf
     */
    'thumbnail'?: string;
}

type FacebookAdsListAds200ResponseFacebookAdsInnerAllOfTypeEnum = 'regular' | 'email-touchpoint' | 'plaintext' | 'rss' | 'reconfirm' | 'variate' | 'absplit' | 'automation' | 'facebook' | 'google' | 'autoresponder' | 'transactional' | 'page' | 'website' | 'social_post' | 'survey' | 'customer_journey' | 'sms'
type FacebookAdsListAds200ResponseFacebookAdsInnerAllOfStatusEnum = 'save' | 'paused' | 'schedule' | 'scheduled' | 'sending' | 'sent' | 'canceled' | 'canceling' | 'active' | 'disconnected' | 'somepaused' | 'draft' | 'completed' | 'partialRejected' | 'pending' | 'rejected' | 'published' | 'unpublished'


