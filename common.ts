...
import { readableStreamToString, MailchimpError, parseIfJson } from "./error";
        ...
        if (e instanceof AxiosError && e.isAxiosError) {
            try {
                const responseBody =
                    ...
                    : e.response?.data
                throw new MailchimpError(e, parseIfJson(responseBody))
            } catch (innerError) {
                if (innerError instanceof ReferenceError) {
                    ...
                    throw new MailchimpError(e, e.response?.data)
                }
                if (innerError instanceof MailchimpError) {
                    // Got "MailchimpError" from the above try block
                    throw innerError;
                }
